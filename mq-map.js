    ...
    _leafletURL: function(B) {
        if (B) {
            ...
            if (/\.\{ext\}/.test(B)) {
                ...
                B = B.replace("{y}", "{y}" + this._config.hd)
            }
        }
        return B
    },
...
MQ.Control.Attribution = L.Control.extend({
    ...
    refreshAttribution: function() {
        ...
        var Q, O = [],
            X, T = this.list,
            R, V = new Date().getFullYear(),
            P = ["©" + V + " MapQuest"],
            U = null,
            S, W, Y = {}, N = window.location.hostname.match(/.ca$/) ? "http://info.mapquest.com/mapquest-terms-of-use-ca-en/" : "http://hello.mapquest.com/terms-of-use";
        ...
        for (X = 0; X < O.length; X++) {
            ...
            if (R.html) {
                switch (R.text_short) {
                    case "MB":
                        P.push('<a href="http://hello.mapquest.com/attributions-mapbox">© Mapbox</a>');
                        break;
                    case "OSM":
